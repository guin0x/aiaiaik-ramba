# This module contains the functions used for the data analyisis of the satellite images
...
def show_image_array(path, scaled_classes=True, cmap='gray', vmin=0, vmax=2, show=True, save_img=False):
    '''
    Load and show a single image using Gdal library. Convert and return it into a numpy array with dtype = np.float32.
    ...
    Can update the pixel values by subtracting 1 from the whole image (element-wise operation) in order to later implement an algorithm that
    ...
    Inputs: 
           ...
           vmin = int, minimum value needed for visualization.
                  default: 0, can range from 0 to 2.  
           vmax = int, maximum value needed for visualization.
                  default: 2, can range from 0 to 2. 
...
def clear_full_df(train_val_test, nodata_thr, water_thr, cols=['Date image', 'no-data: 0', 'non-water: 1', 'water: 2'], 
    ...
    '''
    Select all images that respect the given amount of pixels data threshold (no data max value and water min value).
